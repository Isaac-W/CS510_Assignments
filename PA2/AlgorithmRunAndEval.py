...
def processAndAnalyzeVideo( truth_cap, input_cap, csv_writer, no_out, no_csv,
        doEval, model, writer, height, width ):
...
def main():
    if len(sys.argv) < MIN_ARGS + 1:
        print 'usage: %s <ground truth> <input video> [-e] [-s|-n]' % sys.argv[0]
        print '-----------------------------------------------'
        print 'flags:'
    ...
    sourceName, file_extension = os.path.splitext(input_path)
    ...
    frame_number, videoStats = \
        processAndAnalyzeVideo( truth_cap, input_cap, csv_writer, no_out,no_csv,
            doEval, model, writer, height, width )
    ...
    if not no_out and doEval:
        e.printSummary( frame_number, results )

    # Write video stats
    if not no_csv and doEval:

...
def transform_video(frame, width, height, transform):
    return cv2.warpPerspective(frame, transform, (width, height))


def gaussian_filter(frame, width, height):
    return cv2.GaussianBlur(frame, (11, 11), 2)


# Applies a specified filter to a video.
#
# filter_fcn must have frame as the first argument, followed by the width, height, and other arguments.
# Function must return the processed frame back to the caller.
#
# Example: filter_function(frame, width, height, ...)
def applyVideoFilter(source_path, output_path, filter_fcn=None, *args, **kwargs):
    ...
    while True:
        ...
        if filter_fcn is None:
            filter_fcn = transform_video

        # Apply specified filter (unpack arguments)
        frame = filter_fcn(frame, width, height, *args, **kwargs)
...
def main():
    ...
    # Apply transformation filter--pass in the additional arguments after the function name
    applyVideoFilter(source_path, output_path, transform_video, transform)

    if flag is 'f':
        applyFourierTransform(source_path)

    if flag is 'g':
        applyVideoFilter(source_path, 'GaussianFiltered.avi', gaussian_filter)
